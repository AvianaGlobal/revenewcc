...
def main():
    ...
    # Configure GUI
    parser = GooeyParser()
    grp1 = parser.add_argument_group(gooey_options={'show_border': True})
    grp1.add_argument('dsn', metavar='ODBC Data Source Name (DSN)',
                        help='Please enter the DSN below', action='store')
    grp2 = parser.add_argument_group(gooey_options={'show_border': True})
    grp2.add_argument('--database', metavar='Database Name', widget='TextField',
                     help='Please enter the client database name', action='store')
    grp3 = parser.add_argument_group(gooey_options={'show_border': True})
    grp3.add_argument('--modelfile', metavar='Saved Model', widget='FileChooser',
                     help='Please locate the saved model file', action='store',)
    ...
    from revenewSPR.preprocessing.lists.sum_list import sum_list
    from revenewSPR.preprocessing.lists.min_list import min_list
    from revenewSPR.preprocessing.lists.max_list import max_list
    from revenewSPR.preprocessing.lists.std_list import std_list
    from revenewSPR.preprocessing.lists.mean_list import mean_list
    from revenewSPR.preprocessing.lists.log_list import log_list
    from revenewSPR.preprocessing.top_features import get_top_features
    ...
    duplicate_reports_query = open(application_path + '/revenewSPR/preprocessing/sql/duplicate_reports.sql').read().format(
    ...
    vendor_profiles_query = open(application_path + '/revenewSPR/preprocessing/sql/vendor_profiles.sql').read().format(
    ...
    count_profiles_query = open(application_path + '/revenewSPR/preprocessing/sql/count_profiles.sql').read().format(
        database, duplicates_table, database, duplicates_table)  # Set database and table names for query here
